...
export default function BookFutsal() {
  ...
  const getBookings = async () => {
    try {
      ...
      const booker = check?.data?.data.map((items, id) => items.bookedBy);
      await showBookers(booker);
    } catch (error) {
  ...
  const showBookers = async (id) => {
    try {
      console.log(`${id}`);
      const hi = await axios.get(
        `http://localhost:3000/api/v1/owner/showBookers/${id}`,
        {
          withCredentials: true,
        }
      );
